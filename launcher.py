...
while True:
    user_action = input("Выберите команду:\n"
                        "power up - запустить сервер и клиентов\n"
                        "close - закрыть все окна\n"
                        "exit - выход\n"
                        "Ввод: ")
    if user_action == "power up":
        ...
        for i in range(clients_num):
            proc_list.append(subprocess.Popen(f"python client.py -n user{i+1}", creationflags=subprocess.CREATE_NEW_CONSOLE))
